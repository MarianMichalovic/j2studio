  ...
  return (
    <section ref={heroRef}>
      <div className="grid lg:grid-cols-2 min-h-screen">
        ...
        <div
          className={`relative  h-screen sm:-mt-[95px] sm:mt-0 mt-20 transition-all duration-1000 delay-300 overflow-hidden ${heroInView ? "opacity-100 translate-x-0" : "opacity-0 translate-x-8"}`}
        >
          {/* Slideshow Images */}
          {[
            "/slider/1.jpg",
            ...
            {[0, 1, 2].map((index) => (
              <button
                ...
                className={`w-2 h-2 rounded-full transition-all duration-300 ${
                  currentSlide === index ? "bg-gray-800 w-8" : "bg-gray-700/50"
                }`}
              />
            ))}
          </div>
